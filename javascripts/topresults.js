...
function constructTable(division, scenario, model, metric, result) {
	...
	let system_json_link = row['Details'].replace("/results/", "/systems/") + ".json";
	// let system_json_link = row['Details'].replace("/results/", "/systems/").replace("submissions_automotive_v0.5", "automotive_results_v0.5") + ".json"
...
$(document).ready(function() {
    ...
    readAllData().then(function(global_data) {
        ...
        category = "adas";
        keys = ["Model", "Scenario"];
        values = ["bevformer", scenario];
        myData = filterData(myData, keys, values);
        //console.log(myData);
        $("#model").append('<option selected value="bevformer">Bevformer</option>');
        var scenarios = validScenarios["adas"];// getUniqueValues(myData, "Scenario");
        model = $("#model").val();
        //updateScenarioUnits(myData);
